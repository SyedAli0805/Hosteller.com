    ...
    <div class="d-flex justify-content-center align-items-center min-vh-100">
        <div class="card">
            <div class="card-body">
                ...
                <form action="{{ url('updateBooking/' . $booking->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                    @auth
                    ...
                    <div class="mb-3">
                        ...
                        <input type="number" class="form-control" id="no_of_rooms" name="no_of_rooms" value="{{ $booking->no_of_rooms }}" required>
                        @error('no_of_rooms')
                            <div class="text-danger">{{ $message }}</div>
                        @enderror
                    </div>
                    <button type="submit" class="btn btn-success btn-block">Update Booking</button>
                </form>
            </div>
        </div>
    </div>
